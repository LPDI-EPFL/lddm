...
AUXILIARY_COLUMNS = ['name', 'sample', 'sdf_file', 'pdb_file', 'subdir', 'split', 'cluster']
...
def get_data_type(key: str, data_types: Dict[str, Type], default=None) -> Type:
    ...
    for data_type_key, data_type_value in data_types.items():
        if re.match(data_type_key, key) is not None:
            if found_data_type_key is not None and data_type_value != found_data_type_value:
                curr_prefix = data_type_key.replace('*', '')
                found_prefix = found_data_type_key.replace('*', '')
                if curr_prefix.startswith(found_prefix):
                    # Current key is more specific
                    found_data_type_value = data_type_value
                    found_data_type_key = data_type_key
                    continue
                if found_prefix.startswith(curr_prefix):
                    # Previously found key is more specific
                    continue
                raise ValueError(
                    f'Multiple data type keys with different values match [{key}]:'
                    f' {found_data_type_key} ({found_data_type_value}), {data_type_key} ({data_type_value})'
                )
...
def convert_data_to_table(data: List[Dict], data_types: Dict[str, Type]) -> pd.DataFrame:
    """
    Converts data from `evaluate_samples` to a detailed table
    """
    table = []
    for entry in data:
        table_entry = {}
        for key, value in entry.items():
            if key in AUXILIARY_COLUMNS:
                ...
                continue
            if get_data_type(key, data_types, default=float) != list:
                table_entry[key] = value
...
def aggregated_metrics(table: pd.DataFrame, data_types: Dict[str, Type], validity_metric_name: str = None) -> pd.DataFrame:
    """
    Args:
    ...
    if len(table) == 0:
        logging.warning('Empty table provided for aggregation. Returning empty DataFrame.')
        return pd.DataFrame(columns=['metric', 'value', 'std'])
    ...
    for column in table.columns:
        if column in AUXILIARY_COLUMNS + [validity_metric_name]:
            continue
        col_dtype = get_data_type(column, data_types, default=float)
        if col_dtype not in [bool, int, float]:
            continue
...
def collection_metrics(
    ...
    smiles = table['representation.smiles'].dropna().values
    if len(smiles) == 0:
        logging.warning('No valid input molecules')
...
def evaluate_samples_subdir(
    ...
    for i in tqdm(range(upper_bound), desc=desc, file=sys.stdout):
        ...
        if not in_prot.exists():
            in_prot = Path(in_dir, f'pocket.pdb')  # default to global definition
        if not in_mol.exists() or not in_prot.exists() or \
           in_mol.stat().st_size == 0 or in_prot.stat().st_size == 0:
            print(f'File {in_mol}/{in_prot} does not exist or is empty')
            continue
...
def evaluate_samples(
    ...
    for subdir in in_dir.glob("[!.]*"):
        ...
        curr_data = evaluate_samples_subdir(
            in_dir=subdir,
            evaluator=evaluator,
...
def compute_all_metrics(
        in_dir: Path,
        gnina_path: Path,
        compound_db_path: Path = None,
        reduce_path: Path = None,
        reference_smiles_path: Path = None,
        reference_mols_validity3d: Path = None,
        n_samples: int = None,
    ...
    evaluator = FullEvaluator(gnina=gnina_path, 
                              reference_mols=compound_db_path,
                              reference_mols_validity3d=reference_mols_validity3d,
                              reduce=reduce_path, 
                              exclude_evaluators=exclude_evaluators)
    data = evaluate_samples(in_dir=in_dir, evaluator=evaluator, n_samples=n_samples, job_id=job_id, n_jobs=n_jobs)
    ...
    if reference_smiles_path is not None and len(table_detailed) > 0:
