...
def scalar_cat(x: tuple, *scalars, dim=-1):
    return torch.cat([x[0], *scalars], dim=dim), x[1]


def vector_cat(x: tuple, *vectors, dim=-2):
    return x[0], torch.cat([x[1], *vectors], dim=dim)


def tuple_clone(t):
    '''
    Applies elementwise .clone() to the tuple
    '''
    return tuple(x.clone() for x in t)
...
def ensure_tuple(x: Union[torch.Tensor, tuple[torch.Tensor, torch.Tensor]]):
    """Returns tuple format (s, V) for GVPs. If s or V are missing, they are initialised as 0-element tensors."""
    if isinstance(x, torch.Tensor):
        if x.dim() == 3:
            # x is vector component
            x = (torch.zeros(x.size(0), 0, dtype=x.dtype, device=x.device), x)
        else:
            # x is scalar component
            x = (x, torch.zeros(x.size(0), 0, 3, dtype=x.dtype, device=x.device))
    return x
