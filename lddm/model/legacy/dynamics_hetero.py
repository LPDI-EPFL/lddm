...
from lddm.model.legacy.dynamics import DynamicsBase
from lddm.model import gvp
from lddm.model.gvp import GVP, _rbf, _normalize, tuple_index, tuple_sum, _split, tuple_cat, _merge
...
class DynamicsHetero(DynamicsBase):
    def __init__(self, atom_nf, residue_nf, bond_dict, pocket_bond_dict,
        ...
        if self.condition_time:
            ...
            # from lddm.model.utils import SinusoidalPositionEmbeddings
        ...
        distance_emb_dim = 1 if self.num_rbf is None else self.num_rbf
        _edge_ligand_in = (self.bond_nf + distance_emb_dim, 1 if self.reflection_equiv else 2)
        ...
        if self.enable_masked_modeling:
            _edge_ligand_in = tuple_sum(_edge_ligand_in, (1, 0))

        _n_dist_residue = _residue_atom_dim ** 2 if self.add_all_atom_diff else 1
        _edge_pocket_in = (_n_dist_residue * distance_emb_dim + self.pocket_bond_nf, _n_dist_residue)
        ...
        _ligand_nobond_nf = self.bond_nf + self.enable_masked_modeling + (_edge_ligand_out[0] if self.self_conditioning else 0)
    ...
    # TODO: move all the self-conditioning logic to Dynamics.forward so that we don't need the h_atoms_sc, e_atoms_sc and h_residues_sc inputs
    def _forward(self, x_atoms, h_atoms, mask_atoms, pocket, t, bonds_ligand=None,
        ...
        if bonds_ligand is not None:
            ...
            ligand_bond_types = bonds_ligand[1]

            if self.enable_masked_modeling:
                known_e_feat = known_e.float().unsqueeze(-1)
                ligand_bond_types = torch.cat([ligand_bond_types, known_e_feat], dim=-1)

            # make sure messages are passed both ways
            ligand_edge_indices = torch.cat([ligand_bond_indices, ligand_bond_indices.flip(dims=[0])], dim=1)
            ligand_edge_types = torch.cat([ligand_bond_types, ligand_bond_types], dim=0)
        ...
        if self.enable_masked_modeling:
            ...
            h_atoms = (torch.cat([h_atoms[0], known_x_feat, known_h_feat], dim=1), h_atoms[1])
        ...
        if self.model in {'gvp', 'gvp_v2', 'gvp_transformer'}:
        ...
        edge_logits = torch.zeros(
            ...
            dtype=edge_final.dtype, device=mask_atoms.device
        )
    ...
    def get_edges(self, x_ligand, h_ligand, batch_mask_ligand, edges_ligand, edge_feat_ligand,
        ...
        if self.edge_cutoff_i is not None and len(x_pocket) > 0:
            # _r, _c = radius(x_pocket, x_ligand, r=self.edge_cutoff_i, 
